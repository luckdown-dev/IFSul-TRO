...
void loop(){
    exibeNibbleBinario(243, 1);
    atualizaDisplayComDelay(500);
    exibeNibbleBinario(173, 0);
    atualizaDisplayComDelay(500);
}

void exibeNibbleBinario(int numero, int opcao){
  int nibble;
  int digito;

  if(opcao == 1){
    nibble = (numero & 240) >> 4;  // compara o valor de entrada com a soma dos 4bits mais significativos, após isso desloca os mesmo 4 vezes para a direita a fim de transformar em um valor de 4 digitos
  } else if(opcao == 0) {
    nibble = numero & 15;
  }
